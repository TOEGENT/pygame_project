...
def ai(ball: Ball.Ball, neighbours: list, game):
    ...
    wall_scale_x = config.WINDOW_WIDTH * config.FOOD_DISTANCE_FACTOR
    wall_scale_y = config.WINDOW_HEIGHT * config.FOOD_DISTANCE_FACTOR
    command_scale = max(config.WINDOW_WIDTH, config.WINDOW_HEIGHT)

    #reflex_intent (begin)
    total_ball_intent = Intent([0,0],config.COLOR_BLACK)
    total_food_intent = Intent([0,0],config.COLOR_GREEN)
    ball_vectors = []
    for neighbour in neighbours:
        ...
        #ball_intent (begin)
        ball_factor=1
        future_ball_intent = Intent((0,0),config.COLOR_GREEN)
    ...
    wall_intent = Intent([0,0],config.COLOR_WHITE)
    ...
    if wall_dist_left<wall_scale_x:
        wall_intent.pos[0]+=calc_importance_factor(wall_dist_left, wall_scale_x)
    if wall_dist_top<wall_scale_y:
        wall_intent.pos[1]+=calc_importance_factor(wall_dist_top, wall_scale_y)
    if wall_dist_right<wall_scale_x:
        wall_intent.pos[0]-=calc_importance_factor(wall_dist_right, wall_scale_x)
    if wall_dist_down<wall_scale_y:
        wall_intent.pos[1]-=calc_importance_factor(wall_dist_down, wall_scale_y)    
    #wall_intent (end)
    
    #wander_intent (begin)
    ball.wander_angle += random.uniform(-0.3,0.3)
    wander_intent = Intent([0,0],config.COLOR_WHITE)
    ...
    command_intent = Intent([0, 0], config.COLOR_ORANGE)

    if player_control:
        command_factor = (1 + more_follow - more_unfollow - more_ignore)
        command_dist, command_normal = calc_normal(ball.pos, mouse_pos)
        command_intent.pos[0] += command_normal[0] * command_factor * calc_importance_factor(command_dist, command_scale)
        command_intent.pos[1] += command_normal[1] * command_factor * calc_importance_factor(command_dist, command_scale)
        ...
        if own_blob is not None and enemy_blob is not None:
            team_command_factor = calc_intent(own_blob, enemy_blob, 0, 0) 
            command_dist, command_normal = calc_normal(ball.pos, enemy_blob.pos)
            command_intent.pos[0] += command_normal[0] * team_command_factor * calc_importance_factor(command_dist, command_scale)
            command_intent.pos[1] += command_normal[1] * team_command_factor * calc_importance_factor(command_dist, command_scale)
    #command_intent (end)
    ...
    total_intent = Intent([0, 0], config.COLOR_GREEN)
    ...
    ball.smooth_total_intent = Intent(smooth_add_pos(ball.smooth_total_intent.pos,
